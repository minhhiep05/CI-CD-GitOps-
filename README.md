...
flowchart TB
    ...
    GitLabRepo["📁 GitLab: app-code"]
    HelmRepo["📁 GitLab: helm-chart"]

    subgraph ControlVM["🖥️ control-vm (bastion)"]
        TF["Terraform"]
        KubectlCLI["kubectl / helm / kubeseal"]
    end

    subgraph GitLabVM["🦊 gitlab-vm (EC2 + EIP)"]
        GitLab["GitLab CE + Runner"]
    end

    subgraph HarborVM["📦 harbor-vm (EC2 + EIP)"]
        Harbor["Harbor Registry"]
    end

    subgraph K8sCluster["☸️ Kubernetes Cluster (kubeadm, 3 AZ)"]
        direction TB
        NLB["Network Load Balancer
        (API server, internal)"]
        subgraph Masters["3x Master (control-plane)"]
            M1["master-1"]
            M2["master-2"]
            M3["master-3"]
        end
        subgraph Workers["2x Worker"]
            W1["worker-1
        ...
        + Let's Encrypt"]
        ArgoCD["🔄 ArgoCD + Sealed Secrets"]

        subgraph Envs["📦 myapp-dev / staging / prod"]
            AppPods["App Pods"]
        end
    ...
    S3["🪣 S3
    (Terraform state + Velero backups)"]
    Telegram["✈️ Telegram Bot"]

    Developer -->|"git push"| GitLabRepo
    GitLabRepo -->|"CI: build, scan, push"| Harbor
    GitLabRepo -->|"update image tag"| HelmRepo
    HelmRepo -.->|"detect change"| ArgoCD
    ArgoCD -->|"auto sync"| AppPods
    Harbor -.->|"pull image"| AppPods

    TF -->|"provisions"| K8sCluster
    TF -->|"provisions"| GitLabVM
    TF -->|"provisions"| HarborVM
    TF -.->|"state"| S3
    KubectlCLI -->|"private IP, same VPC"| NLB
    NLB --> Masters

    User -->|"HTTPS gitlab.domain.com"| GitLab
    User -->|"HTTPS harbor.domain.com"| Harbor
    User -->|"HTTPS argocd.domain.com"| W1
    CertManager -.->|"issues TLS cert"| ArgoCD
    W1 --> ArgoCD
    ...
    class NLB,CertManager trafficStyle
    class AppPods podStyle
    class ArgoCD,Velero controlStyle
    class Prom,Grafana,Loki,AM monitorStyle
    class GitLabRepo,HelmRepo,GitLab,Harbor,TF,KubectlCLI cicdStyle

    style ControlVM fill:#F1F8E9,stroke:#558B2F,stroke-width:3px
    style GitLabVM fill:#F1F8E9,stroke:#558B2F,stroke-width:3px
    style HarborVM fill:#F1F8E9,stroke:#558B2F,stroke-width:3px
    style K8sCluster fill:#F1F8E9,stroke:#558B2F,stroke-width:3px
    style Observability fill:#FFFDE7,stroke:#FBC02D,stroke-width:1.5px
    style Envs fill:#E8F5E9,stroke:#43A047,stroke-width:1.5px
    style Masters fill:#E3F2FD,stroke:#1976D2,stroke-width:1.5px
    style Workers fill:#E3F2FD,stroke:#1976D2,stroke-width:1.5px
```
